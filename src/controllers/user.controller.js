...
const registerUser = asyncHandler( async (req, res) => {
    ...
    if ([fullName, email, username, password].some((field)=>{field?.trim()===""})) {
        throw new ApiError(400, "All fields are required")              // validation - not empty
    }

    const existedUser = await User.findOne({$or: [{username}, {email}]})  // check if user already exists: username, email
    if(existedUser)
    {
    ...
    const createdUser = User.findById(user._id).select("-password -refreshToken")   // check for user creation ; remove password and refresh token field from response
    if (!createdUser) {
